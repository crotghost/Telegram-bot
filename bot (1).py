...
async def handle_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        # Track this join request so photos can be linked back to it
        context.application.bot_data.setdefault("pending_verifications", {})[user.id] = chat_id
        ...
        # Try to DM the user immediately with verification instructions
        dm_sent = False
        try:
            await context.bot.send_message(
                chat_id=user.id,
                text=(
                    "👋 Ciao! Abbiamo ricevuto la tua richiesta di accesso al canale.\n\n"
                    "Per completare la verifica inviaci due foto:\n\n"
                    "1️⃣ Uno screenshot del tuo profilo Instagram\n"
                    "2️⃣ Un selfie con il documento d'identità in mano\n\n"
                    "Invia la prima foto quando sei pronto. 📸"
            ...
            dm_sent = True
            logger.info(f"Auto-DM sent to user={user.id}")
        except Exception as e:
            logger.info(f"Auto-DM failed for user={user.id} (hasn't started bot): {e}")

        if dm_sent:
            # Verification already started — admin just needs Approve / Reject
            keyboard = InlineKeyboardMarkup([[
            ...
            admin_text = (
                ...
                f"{user_info}\n\n"
                f"✉️ <i>Istruzioni di verifica inviate automaticamente all'utente.</i>\n\n"
                f"Attendi le foto e poi approva o rifiuta."
            )
        else:
            # Bot can't DM the user — give admin a shareable deep link
            try:
                bot_me    = await context.bot.get_me()
                param     = f"jc_{chat_id}_{user.id}"
                deep_link = f"https://t.me/{bot_me.username}?start={param}"
            except Exception:
                deep_link = "Non disponibile"
            ...
            admin_text = (
                f"📥 <b>Nuova richiesta di accesso al canale</b>\n\n"
                f"{user_info}\n\n"
                f"⚠️ <i>Impossibile contattare l'utente — non ha ancora avviato il bot.</i>\n\n"
                f"📩 <b>Invia questo link all'utente per avviare la verifica:</b>\n"
                f"<code>{deep_link}</code>\n\n"
                f"Oppure usa il pulsante 'Avvia Verifica' per riprovare."
            )

        await _notify_admins(context, admin_text, keyboard)
